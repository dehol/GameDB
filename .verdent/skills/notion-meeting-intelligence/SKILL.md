...
description: Prepare meeting materials with Notion context and AI research; use when gathering context, drafting agendas/pre-reads, and tailoring materials to attendees.
license: MIT
metadata:
  version: '1.0.1'
---
...
Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with AI research.
...
5) Enrich with AI research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.

## Workflow
### 0) If any MCP call fails because Notion MCP is not connected, pause and set it up:

The Notion MCP server must be configured for your agent. Refer to your agent's MCP documentation for setup instructions:

1. **Add the Notion MCP server** with URL: `https://mcp.notion.com/mcp`
2. **Enable remote MCP client** if required by your agent.
3. **Authenticate** with Notion via OAuth.

After successful setup, the user may need to restart their agent session. Tell them so, and when they return they can continue with Step 1.
...
- Add concise AI research where helpful: market/industry facts, benchmarks, risks, best practices.
